...
def do_insert_product_info_tab():
    ...
    while True:
        ...
        try:
            con.short_execute_sql(DiyBusiness().product_info_tab())
            con.short_execute_sql(DiyBusiness().product_info_tab_clob())
            i = i + 1
            if i == 1000:
                socket_client_for_statistics('192.168.3.118', 'do_insert_product_info_tab--' + str(i), 45678)
                i = 0
        except Exception:
            con_list = sel_con()
            if len(con_list) != 0:
                ...
                print('No Useable IpAddress!')
                socket_client_for_statistics('192.168.3.118', 'do_insert_cmcc_user_info--' + str(i), 45678)
                break
